...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const dsaSnap = await adminDB.collection("dsa_questions").where("id", "==", docId).limit(1).get();
    ...
    if (!dsaSnap.empty) {
        ...
        const csSnap = await adminDB.collection("cs_topics").where("id", "==", docId).limit(1).get();
